...
function Model(props) {
  const { nodes, materials } = useGLTF('/src/public/models/scene.glb')
    ...
    if (props.item && props.item.color) {
      Object.entries(materials).map((material) => {
        if (
          ...
          const color = props.item.color[0]; // Ensure color exists
          if (color) {
...
useGLTF.preload('/src/public/models/scene.glb')